...
with left_col:
    ...
    route = st.selectbox("Route of administration", ["IV bolus", "IV infusion", "Extravascular (oral/other)"], key="route")

    if route in ["IV bolus", "IV infusion"]:
        F = 1.0
        ka = None
        if route == "IV infusion":
            Tinf = st.number_input("Infusion duration $T_{inf}$ (h)", 0.01, 24.0, st.session_state.get("Tinf", 1.0), key="Tinf")
        else:
            Tinf = None
    else:
        bio_method = st.radio(
            "Specify bioavailability directly or calculate from $F_a$, $F_g$, and $E_h$?",
            ["Directly specify F", "Calculate F"], key="bio_method"
        )
    ...
    Vd_mode = st.radio("Volume of distribution $V_d$ mode", ["Fixed $V_d$", "Calculate from $f_u/f_{ut}$"], key="Vd_mode")
    ...
    use_global_pk = st.radio("Input mode:", ["Global Parameter Mode", "Renal-Hepatic Mode"], key="pk_mode")
    ...
    if use_global_pk == "Global Parameter Mode":
        pk_choice = st.selectbox("Specify parameter", ["Clearance", "Half-life", "Elimination rate constant"], key="pk_choice")
        if pk_choice == "Clearance":
            CL = st.number_input(r"Clearance $CL$ (L/h)", 0.0, 100.0, st.session_state.get("CL", 5.0), key="CL")
            k = CL / max(Vd, EPS)
            t_half = float("inf") if k <= EPS else np.log(2) / k
        elif pk_choice == r"Half-life":
            t_half = st.number_input(r"Half-life $t_{1/2}$ (h)", 0.0, 1000.0, 8.0, key="t_half")
            k = np.log(2) / max(t_half, EPS) if t_half > 0 else 0.0
            CL = k * Vd
        else:
            k = st.number_input(r"Elimination rate constant $k_{el}$ (h⁻¹)", 0.0, 5.0, 0.1, key="kel")
            CL = k * Vd
        ...
        if include_renal:
            ...
            GFR_mL_min = st.slider(r"GFR (mL/min)", 0.0, 120.0, 100.0, step=1.0, key="GFR")
            CL_adj_mL_min = st.slider("Secretion/Reabsorption adj. (mL/min)", -120.0, 600.0, 0.0, step=1.0, key="CL_adj")
            GFR_L_h = GFR_mL_min * 60 / 1000
            CL_adj_L_h = CL_adj_mL_min * 60 / 1000
            CL_renal = max(fu * GFR_L_h + CL_adj_L_h, 0.0)
            st.write(f"Calculated renal clearance = {CL_renal:.2f} L/h")

        if include_hepatic:
            st.subheader(r"Hepatic Clearance via $Q_h$ and $CL_{int}$")
            Qh = st.number_input(r"Hepatic blood flow $Q_h$ (L/h)", 0.1, 150.0, 90.0, key="Qh")
            CLint = st.number_input(r"Intrinsic clearance $CL_{int}$ (L/h)", 0.1, 500.0, 200.0, key="CLint")
            CL_hepatic = (Qh * fu * CLint) / (Qh + fu * CLint)
            Eh = CL_hepatic / Qh
            st.write(f"Hepatic extraction ratio $E_h$ = {Eh:.2f}")
        ...
        if include_other:
            ...
            if "enz_name" not in st.session_state:
                st.session_state["enz_name"] = ""
            if "enz_cl" not in st.session_state:
                st.session_state["enz_cl"] = 0.0

            def _add_enzyme():
                name = st.session_state["enz_name"].strip()
                val = float(st.session_state["enz_cl"])
                if name:
                    st.session_state["enzymes"][name] = val
                    st.session_state["enz_name"] = ""
                    st.session_state["enz_cl"] = 0.0

            def _remove_enzyme(name: str):
                st.session_state["enzymes"].pop(name, None)

            with st.expander("Add/remove enzyme contributions (L/h)"):
                st.text_input("Enzyme name", key="enz_name")
                st.number_input("Enzyme CL (L/h)", 0.0, 50.0, key="enz_cl")
                st.button("Add enzyme", on_click=_add_enzyme, key="add_enzyme")
                if st.session_state["enzymes"]:
                    for enz, val in list(st.session_state["enzymes"].items()):
                        cols = st.columns([3, 1])
                        cols[0].write(f"{enz}: {val:.2f} L/h")
                        cols[1].button(f"Remove {enz}", on_click=_remove_enzyme, args=(enz,), key=f"remove_{enz}")
    ...
    # ---------- DOSING ----------
    st.subheader("Dosing")
    dose = st.number_input("Dose (mg)", 0.0, 20000.0, st.session_state.get("dose", 100.0), key="dose")
    tau = st.number_input("Dosing interval (h)", 0.01, 168.0, st.session_state.get("tau", 12.0), key="tau")
    n_doses = int(st.number_input("Number of doses", 1, 100, st.session_state.get("n_doses", 5), key="n_doses"))
    t_end = tau * n_doses
...
with right_col:
    ...
    def conc_extravascular(dose, Vd, k, ka, F, t, tau, n_doses):
        ...
        for n in range(n_doses):
            ...
            mask = (t >= n * tau)
            if ka is None:
                raise ValueError(r"$k_a$ must be provided")
            if abs(ka - k) > EPS:
